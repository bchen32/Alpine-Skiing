...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_pressed('move_left') and angle_diff < max_angle_diff:
		velocity = velocity.rotated(Globals.normal, PI * inv_angle_diff * turn_vel_coeff * delta)
	if Input.is_action_pressed('move_right') and angle_diff > -max_angle_diff:
		velocity = velocity.rotated(Globals.normal, -PI * inv_angle_diff * turn_vel_coeff * delta)
	# Return to center force
	velocity = velocity.rotated(Globals.normal, -PI * inv_angle_diff * turn_vel_coeff * angle_diff * rtc_vel_coeff * delta)
